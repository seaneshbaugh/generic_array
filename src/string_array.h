...
#define __GENERIC_ARRAY_NAME__ StringArray

#define GENERIC_ARRAY_VALUES_EQUAL(a, b) strncmp(a, b, strlen(a) < strlen(b) ? strlen(a) : (strlen(b))) == 0
...
#define GENERIC_ARRAY_ELEMENT_TO_STRING_BUFFER_SIZE(element) (int)strlen(element) + 5

#define GENERIC_ARRAY_ELEMENT_TO_STRING_LENGTH(buffer, bufferSize, element) snprintf(buffer, bufferSize, "\"%s\", ", element)

#define GENERIC_ARRAY_ELEMENT_TO_STRING_LAST_LENGTH(buffer, bufferSize, element) snprintf(buffer, bufferSize, "\"%s\"", element)

#define GENERIC_ARRAY_ELEMENT_TO_STRING(buffer, bufferSize, element) snprintf(buffer, bufferSize, "\"%s\", ", element)

#define GENERIC_ARRAY_ELEMENT_TO_STRING_LAST(buffer, bufferSize, element) snprintf(buffer, bufferSize, "\"%s\"", element)
